...
function summaryFor(checks: OperationalCheck[]): OperationalSummary {
  return checks.reduce<OperationalSummary>((summary, check) => {
...
function runStatus(summary: OperationalSummary): OperationalRunStatus {
...
async function collectChecks(): Promise<OperationalCheck[]> {
  ...
  const [messages, webhooks, campaigns, eligibility, backup] = await Promise.all([
    pool.query<{
      failed_recent: string | number;
      unknown_stale: string | number;
      pending_stale: string | number;
    }>(`
      SELECT
        COUNT(*) FILTER (
        ...
        COUNT(*) FILTER (
          WHERE status = 'UNKNOWN' AND updated_at < NOW() - INTERVAL '15 minutes'
        ) AS unknown_stale,
        COUNT(*) FILTER (
    ...
    `),
    pool.query<{
      errors_recent: string | number;
      unprocessed_stale: string | number;
    }>(`
      SELECT
        COUNT(*) FILTER (
        ...
        ) AS unprocessed_stale
      FROM webhook_events
    `),
    pool.query<{ unsafe_statuses: string | number }>(`
      SELECT COUNT(*) AS unsafe_statuses
      FROM campaigns
      WHERE status NOT IN ('DRAFT','PREVIEWED','CANCELLED')
    `),
    pool.query<{ ineligible_ready: string | number }>(`
      SELECT COUNT(*) AS ineligible_ready
      FROM campaign_recipients cr
      JOIN contacts ct ON ct.id = cr.contact_id
      WHERE cr.status = 'READY'
        AND (
          ct.archived_at IS NOT NULL
          OR ct.commercial_status = 'DO_NOT_CONTACT'
          OR ct.opt_out_at IS NOT NULL
          OR ct.consent_status <> 'GRANTED'
          OR ct.wa_id !~ '^[0-9]{8,20}$'
          OR ct.phone !~ '^[0-9]{8,20}$'
        )
    `),
    pool.query<{
      status: string;
  ...
  const ineligibleReady = numberValue(eligibility.rows[0]?.ineligible_ready);
  const backupRow = backup.rows[0];
  const backupVerifiedAt = backupRow?.verified_at ? new Date(backupRow.verified_at).getTime() : Number.NaN;
  const backupFresh = backupRow?.status === 'SUCCESS'
    && Number.isFinite(backupVerifiedAt)
    && backupVerifiedAt >= Date.now() - 36 * 60 * 60 * 1000;
  ...
  return [
    ...
    },
    {
      ...
      details: {
        latestStatus: backupRow?.status ?? null,
        completedAt: backupRow?.completed_at ?? null,
        verifiedAt: backupRow?.verified_at ?? null
      }
    }
  ];
}

async function updateAlertForCheck(check: OperationalCheck): Promise<void> {
    ...
    await pool.query(
      `UPDATE system_alerts
       SET severity = $2, title = $3, details = $4::jsonb,
           occurrence_count = occurrence_count + 1,
           last_seen_at = NOW(), acknowledged_at = NULL,
           acknowledged_by = NULL, updated_at = NOW()
       WHERE fingerprint = $1 AND resolved_at IS NULL`,
      [fingerprint, check.severity, check.title, JSON.stringify(check.details)]
    );
  }
...
export async function runOperationalChecks(triggerSource: OperationalTrigger): Promise<OperationalRun> {
  ...
  try {
    ...
    for (const check of checks) await updateAlertForCheck(check);
...
async function getAlertById(id: string): Promise<OperationalAlert> {
  const result = await pool.query<AlertRow>(
    `SELECT id, severity, source, title, details, fingerprint,
            occurrence_count, acknowledged_at, resolved_at,
            created_at, last_seen_at, updated_at
